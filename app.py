...
def extract_kpis(text: str) -> pd.DataFrame:
    ...
    if "attrition" in low or "retention" in low:
        rows.append({
            ...
            "Description":"Track voluntary attrition trends and target 10% reduction in 12 months.",
            "Target Value":"10% reduction in 12 months",
            "Status":"Pending"
        })
    rows.append({
        "KPI Name":"System Uptime",
        "Description":"Ensure system availability and scalability to minimize downtime.",
        "Target Value":"99.9%",
        "Status":"Pending"
    })
...
def recommend(domain: str, existing: list, topic: str = None, raw_text: str = "") -> list:
    pool = [
        ("Involuntary Attrition Rate", "Measure attrition due to involuntary exits."),
        ("Employee Retention Rate", "Monitor percentage of employees retained annually."),
        ("First Year Attrition Rate", "Track attrition within first year of employment.")
    ]
    return [
...
# (Tables and forms rendering functions remain unchanged)
# ... KEEP your render_extracted_table, render_recommended_table, manual_kpi_adder as in your version ...
